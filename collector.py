...
def read_device(client, slave_id, config):
    try:
        ...
        # Pymodbus 3.x: 5 register oku (70=güç, 71=voltaj, 72=akım, 73=toplam üretim, 74=sıcaklık)
        rr = client.read_holding_registers(address=config['start_addr'], count=5, slave=slave_id)
        if rr.isError():
            logging.error(f"Modbus okuma hatasi (ID={slave_id}): {rr}")
            try:
                client.close()
            except Exception:
                pass
        ...
        veriler = {
            ...
            "sicaklik": rr.registers[4] * config['isi_scale']  # Register 74 = Sıcaklık
        }
