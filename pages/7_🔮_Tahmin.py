...
st.set_page_config(page_title="Üretim Tahmini", page_icon="🔮", layout="wide")
inject_glossy_css()
if not check_auth():
    st.stop()
logout_button()
...
st.title("🔮 AI Destekli Üretim Tahmini")
section_header("🤖", "Makine Öğrenmesi Modeli", "Geçmiş verilere dayanarak gelecek üretim kapasitesini öngörün")
...
with col_settings:
    hedef_cihaz = st.selectbox("Hedef Cihaz Seçin:", ["Tüm Cihazlar (Ort)"] + [f"ID {s}" for s in slave_ids])
    tahmin_periyodu = st.radio("Tahmin Süresi:", ["Gelecek 24 Saat", "Gelecek 7 Gün"], horizontal=True)
    hava_durumu = st.select_slider("Beklenen Hava Durumu:", options=["Bulutlu (Düşük)", "Parçalı Bulutlu", "Güneşli (Yüksek)"], value="Parçalı Bulutlu")

with col_info:
    st.markdown("""
        <div class="glossy-card" style="padding:15px; margin-top:28px;">
            <div style="font-weight:600; color:#38bdf8; margin-bottom:5px;">ℹ️ Model Bilgisi</div>
            <div style="font-size:0.9rem; color:#94a3b8;">
            <b>Algoritma:</b> Random Forest Regressor<br>
            Bu model son 10.000 veri ölçümünü temel alarak günün saati ve ortam sıcaklığı ile güç üretimi arasındaki ilişkiyi öğrenir. Gelecek projeksiyonu seçilen hava durumuna göre simüle edilmiştir. Eksi değerli veriler filtrelenmiştir.
            </div>
        </div>
...
if st.button("🚀 Tahmini Başlat", type="primary"):
    with st.spinner("Geçmiş veriler analiz ediliyor ve model eğitiliyor..."):
        
        # 1. Veri Hazirligi
        veriler = []
        if hedef_cihaz == "Tüm Cihazlar (Ort)":
        ...
        if len(veriler) < 100:
            st.error("Modeli eğitmek için yetersiz veri! En az 100 kayıt gereklidir.")
        else:
            df = pd.DataFrame(veriler)
            df = df[df['guc'] > 0] # Sifir ve negatifleri dahil etmeyelim
            if len(df) < 50:
                 st.error("Gün ışığında yeterince ölçüm (Güç > 0) bulunamadı.")
            else:
                ...
                now = datetime.now()
                gelecek_verisi = []
                saatlik_nokta = 60 # 24 saat = 24 nokta, ama biz saat basi 1 veri mi koyalim? evet.
                
                delta_saat = 24 if "24 Saat" in tahmin_periyodu else (24 * 7)
                
                base_temp = 25
                if hava_durumu == "Bulutlu (Düşük)":
                    base_temp = 18
                    guc_carpani = 0.6
                elif hava_durumu == "Güneşli (Yüksek)":
                    base_temp = 32
                    guc_carpani = 1.2
                else:
                    guc_carpani = 0.9
                ...
                for i in range(1, delta_saat + 1):
                    ...
                    # Sicaklik simülasyonu (öğlen sicak, gece soguk)
                    isinma_katsayisi = -math.cos(math.pi * gelecek_saat / 12) if 'math' in locals() else np.cos((gelecek_saat - 14)*np.pi/12)
                    sim_sicaklik = base_temp + (isinma_katsayisi * 7) # gece base_temp-7, gündüz base_temp+7
                ...
                # KPI Kartlari
                kpi_row([
                    {"value": f"{toplam_kwh:.2f} kWh", "label": f"Beklenen Toplam Üretim ({tahmin_periyodu})", "color": "#10b981"},
                    {"value": f"{df_gelecek['tahmini_guc'].max():.0f} W", "label": "Beklenen Tepe Gücü (Peak)", "color": "#f59e0b"},
                ...
                fig.update_layout(
                    ...
                    yaxis=dict(gridcolor='rgba(255,255,255,0.04)', title="Güç Çıkışı (W)"),
                ...
                st.success("Tebrikler! Model geçmiş verilerden bağıntıları öğrenerek hava durumu profiliyle örtüşen sentetik bir üretim tahmini çıkardı.")
